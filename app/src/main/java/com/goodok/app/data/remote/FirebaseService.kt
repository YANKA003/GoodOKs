package com.goodok.app.data.remote
...
class FirebaseService {
    ...
    // Update user's RuStore push token
    suspend fun updatePushToken(uid: String, token: String): Result<Unit> {
        return try {
            database.child("users").child(uid).child("rustoreToken").setValue(token).await()
            Result.success(Unit)
        } catch (e: Exception) {
            Result.failure(e)
        }
